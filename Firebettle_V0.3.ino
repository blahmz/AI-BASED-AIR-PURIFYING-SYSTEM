...
void loop() {
  ...
  // Fan control logic (MODIFIED FOR YOUR REQUIREMENT)
  // Automatic mode: Fan ON if MQ135 #1 (input) > 500 ppm
  bool autoFan = (ppm1 > 1000);
    ...
    while (client.connected() && currentTime - previousTime <= timeoutTime) {
      ...
      if (client.available()) {
        ...
        if (header.indexOf("\r\n\r\n") >= 0) {
          ...
          client.print("<p><b>MQ135 #1 (Input):</b> "); client.print(ppm1, 1); client.println(" ppm</p>");
          client.print("<p><b>MQ135 #2 (Output):</b> "); client.print(ppm2, 1); client.println(" ppm</p>");
          client.print("<p><b>Average:</b> "); client.print(avg_ppm, 1);

          // Display air quality status based on input sensor
          if (ppm1 < 700) {
